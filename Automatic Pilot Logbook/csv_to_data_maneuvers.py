...
# Function to create a map with the flight path and log landings and low passes
def create_flight_path_map(csv_file, airport_coords, output_html, landings_csv, maneuvers_csv, buffer_radius_km=0.5):
    ...
    with open(landings_csv, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['ID', 'State', 'Site', 'Total Landings', 'Day Full Stop Landings', 'Day Touch & Go Landings', 'Night Full Stop Landings', 'Night Touch & Go Landings', 'Total Day Landings', 'Total Night Landings', 'Low Passes'])
        for airport in airport_coords:
            ...
            if total_landings > 0 or low_pass_count > 0:
                writer.writerow([airport['ID'], airport['State'], airport['Site'], total_landings, day_full_stop_count, day_touch_and_go_count, night_full_stop_count, night_touch_and_go_count, total_day_count, total_night_count, low_pass_count])
