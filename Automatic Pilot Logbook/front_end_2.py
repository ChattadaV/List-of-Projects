...
# Updated show_popup method in MainScreen class
class MainScreen(BoxLayout):
    ...
    def show_popup(self, message):
        try:
            ...
            # Display HTML map if possible
            try:
                map_image = Image(source='map_image.png', size_hint=(1, 0.75))  # Replace with map screenshot path
                popup_layout.add_widget(map_image)
            except Exception as e:
                popup_layout.add_widget(Label(text="Map display not available"))

            # Scrollable view for formatted CSV data
            csv_scroll = ScrollView(size_hint=(1, 0.25))
            csv_layout = GridLayout(cols=2, spacing=5, size_hint_y=None)
            csv_layout.bind(minimum_height=csv_layout.setter('height'))

            # Sample formatted rows for the CSV data
            headings = [
                "Date", "Make and Model", "Tail Number", "From", "Route", "To", "Day Landings", "Night Landings",
                "Instrument Approaches", "Type/Location", "Single Time", "Multi Time",
                "Actual Time", "Simulated Hood Time", "FTD/Simulator Time",
                "Night Time", "Cross Country Day (All)", "Cross Country Day (>50 NM)",
                "Cross Country Night (All)", "Cross Country Night (>50 NM)",
                "PIC Time", "Solo Time", "Ground Training Received", "Flight Training Received",
                "Training Given", "Total Flight Time", "Remarks"
            ]

            values = [
                "10/10/2024", "a", "N5700W", "KEKY", "KSEM - KEET", "KEKY", "0", "5",
                "1", "b", "2.8", "0",
                "1.0", "2.0", "3.0",
                "2.8", "0", "0",
                "2.6", "2.6",
                "2.8", "2.8", "1.0", "3.0",
                "2.0", "2.8", "KEET: 1 night full stop landings..."
            ]

            for i in range(0, len(headings), 2):
                # Displaying headings in 2 columns
                csv_layout.add_widget(Label(text=headings[i], bold=True))
                csv_layout.add_widget(Label(text=values[i]))

                if i + 1 < len(headings):
                    csv_layout.add_widget(Label(text=headings[i + 1], bold=True))
                    csv_layout.add_widget(Label(text=values[i + 1]))

            csv_scroll.add_widget(csv_layout)
            popup_layout.add_widget(csv_scroll)
